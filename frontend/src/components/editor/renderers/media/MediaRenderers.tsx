...
export function renderCarousel({ block, onUpdate }: MediaRendererProps) {
  ...
  return (
    <div className="space-y-3">
      <CollapsibleSection title="Éléments du carousel" count={carouselItems.length} defaultCollapsed={false}>
        <div className="space-y-3">
          {carouselItems.map((item: any, index: number) => (
            <div key={index} className="border border-gray-300 dark:border-gray-600 rounded-lg p-3 space-y-2">
              <div className="flex items-center justify-between mb-2">
                ...
                <button
                  onClick={() => {
                    const newItems = carouselItems.filter((_: any, i: number) => i !== index)
          ...
          <button
            onClick={() => {
              const newItems = [...carouselItems, { image: '', title: '', description: '' }]
              onUpdate({ data: { ...safeBlock.data, items: newItems } })
            }}
            className="w-full px-2 py-1 text-xs bg-blue-500 text-white rounded hover:bg-blue-600 flex items-center justify-center gap-1"
          >
