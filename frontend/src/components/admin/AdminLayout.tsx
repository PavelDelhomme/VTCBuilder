...
export default function AdminLayout({ children, title, subtitle, headerActions, saveStatus, hideHeader = false, showBackButton = false, backUrl, projectBackButton }: AdminLayoutProps) {
  ...
  // Détecter automatiquement si on est sur une page de détail de projet
  const isProjectDetailPage = pathname?.match(/^\/admin\/projects\/\d+$/) !== null
  const shouldShowBackButton = showBackButton || isProjectDetailPage
  const defaultBackUrl = isProjectDetailPage ? '/admin/projects' : undefined
    ...
    <div className="h-screen bg-gray-100 dark:bg-gray-900 flex flex-col overflow-hidden overflow-x-hidden">
      ...
      <div className="flex relative flex-1 min-h-0 overflow-hidden">
        ...
        <div className={`flex-1 w-full min-w-0 transition-all duration-300 flex flex-col overflow-hidden ${
          ...
          {!hideHeader && (
          <header className={`hidden lg:block flex-shrink-0 transition-colors duration-300 ${
            resolvedTheme === 'dark' 
