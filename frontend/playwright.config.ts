...
export default defineConfig({
  ...
  timeout: 60 * 1000, // Increased to 60s for complex tests
  expect: {
    /**
  ...
  use: {
    ...
    baseURL: process.env.PLAYWRIGHT_BASE_URL || (process.env.CI ? 'http://frontend:3000' : 'http://localhost:9494'),
  ...
  // Désactivé car le frontend est déjà en cours d'exécution dans Docker
  // webServer: {
  //   command: 'npm run dev',
